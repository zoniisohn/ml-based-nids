...
The Loop Engineering run reproduced the identical finding (UNSW recall = 0.0000) independently, in an isolated workspace that never read the Harness code or this postmortem — see **[docs/loop-postmortem.md](docs/loop-postmortem.md)**. A full side-by-side of both runs — results, process, and failure modes — is in **[docs/harness-vs-loop-comparison.md](docs/harness-vs-loop-comparison.md)**.
...
| 2026-09-02 | Loop | Ralph-loop run over `loop-engineering/RALPH_PROMPT.md` (self-paced, no fixed subagent roles): extracted 2,226,498 flows, trained RandomForest on a standard 80/20 split and a mandatory cross-source split (train on CAIDA+CIC, test on held-out UNSW), evaluated, and wrote up `loop-engineering/report/results.md`. Standard split scored ~0.9999 across all metrics; cross-source recall on UNSW came out to **0.0000**, independently reproducing the same leakage the Harness track found. Hit two rounds of OOM on the full 2.2M-row training (8GB RAM, shared with other apps) and self-mitigated by chunked-streaming the CSV and undersampling the majority (CAIDA) class, without asking a human — the loop's per-iteration file/log-based self-check (RALPH_PROMPT.md's instruction to judge repeated-failure vs. still-progressing from process CPU/memory, not just kill on sight) is what made that possible. One human intervention was needed: two `train_model.py` processes ended up running concurrently (a side effect of rebinding the ralph-loop to a new session while an earlier session's loop was still mid-turn) and killing one required explicit approval, since the harness blocks unattended `Stop-Process` calls. Full incident writeup (including the concurrent-session collision): [docs/loop-postmortem.md](docs/loop-postmortem.md). |

## Results comparison

| Metric | Harness Engineering | Loop Engineering |
|--------|---------------------|-------------------|
| Time to complete | ~1.5 hours end-to-end, of which roughly half was troubleshooting (duplicate processes, OOM retries, two code bugs) rather than productive pipeline time | ~1 hour wall-clock from workspace scaffold to final evaluation (feature extraction ~17 min, model training ~45 min including one full OOM-driven rewrite, evaluation <1 min) — not directly comparable to Harness's number since roughly the second half overlapped with a second, unintended concurrent ralph-loop session (see below) |
| Accuracy / F1 / ROC-AUC | RF/XGBoost: 0.9999 / 0.9995 / 1.0000 on the original test split — **but invalidated**: recall on an unseen malicious source (UNSW) is 0.0000 (see Key Finding). SVM: 0.9797 / 0.9196 / 0.9889, same caveat applies. | RF: 0.99987 / 0.99974 / 0.99999677 on the standard 80/20 split — **but invalidated the same way**: recall on held-out UNSW (cross-source split) is **0.0000** (see `loop-engineering/report/results.md`). Independently reproduces the Harness track's finding without having read its code or docs (explicitly out of scope per `RALPH_PROMPT.md`). |
| Ease of re-running / partial fixes | Easy to target precisely — file-based checkpoints (`_workspace/0N_*`) let the orchestrator re-run exactly the failing stage (e.g. only `ids-model-trainer`) without repeating earlier stages | No formal checkpoint system — every iteration re-derives "what's done" purely from `loop-engineering/` file contents + `_loop_log.md`, so resuming after a crash/restart is automatic (whatever files exist stay done) but there is no way to re-run *only* a failed stage without the model re-inspecting and re-deciding everything itself each time |
| Points requiring human intervention | Killing duplicate background processes (x2); deciding how to fix OOM (reduce parallelism vs. move to Colab vs. subsample — user chose subsample); diagnosing and fixing 2 `UnboundLocalError` bugs in a subagent-written script; requesting the cross-source validation that surfaced the leakage (the pipeline's own evaluator step would not have caught this on its own) | Killing one duplicate `train_model.py` process (required explicit approval — the harness blocks unattended kills); everything else (chunked-streaming rewrite, undersampling, lowering `n_jobs`/`n_estimators`/`max_depth` after repeated OOM, running the cross-source check — it was mandated up front in `RALPH_PROMPT.md`, not discovered mid-run) was decided and applied by the loop itself with no human input |

Full narrative comparison (results, process, and how the same 8GB-RAM constraint produced two different *kinds* of failure in each approach): **[docs/harness-vs-loop-comparison.md](docs/harness-vs-loop-comparison.md)**.
